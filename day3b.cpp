...
int main() {
    ...
    for (size_t i = 0; i + 3 < fullContent.size(); ++i) {
        // Set ifDo to true if a do conditional is found
        if (fullContent.substr(i, 4) == "do()") {
            ifDo = true;
        }
        // Set ifDo to false if a don't conditional is found
        if (fullContent.substr(i, 7) == "don't()") {
            ifDo = false;
        }
        
        if ((fullContent.substr(i, 4) == "mul(") && ifDo) {
